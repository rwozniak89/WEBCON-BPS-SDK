...
namespace WebCon.BpsExt.Training.CustomDataSources
{
    public class RequestItemsDataSource : CustomDataSource<RequestItemsDataSourceConfig>
    {
        ...
        private Dictionary<string, string> GetPersons()
        {
            ...
            return DataSourcesHelper.GetDataTableFromDataSource(getdataParams).AsEnumerable().ToDictionary(row => row[Configuration.CategoryColumnName].ToString(), row => row[Configuration.PersonColumnName].ToString(), StringComparer.CurrentCultureIgnoreCase);
        }
