...
namespace WebCon.BpsExt.Training.TaxIdFormField
{
    public class TaxID : FormFieldExtension<TaxIDConfig, TaxIDValue>
    {
        ...
        public override void VerifySaveRestrictions(ControlSaveRestrictionsParams<FormFieldValueExtensionContext<TaxIDValue>> args)
        {

            if (!IsTaxControlValid(args.Context))
            {
                args.IsValid = false;
                args.ErrorMessage = "Invalid value of control TaxID";
            }
        }

        private bool IsTaxControlValid(FormFieldValueExtensionContext<TaxIDValue> context)
        {
            var value = context.GetValue();

            if (value == null || string.IsNullOrEmpty(value.TaxIdentificationNumber))
                return false;

            if (value.TaxIdentificationNumber.Contains("-"))
            {
                value.TaxIdentificationNumber = value.TaxIdentificationNumber.Replace("-", "");
                context.SetFieldValue(value);
            }

            return value.TaxIdentificationNumber.Length == 10;
        }
    }
}
